...
namespace STSdb4.WaterfallTree
{
    public partial class WTree
    {
        private partial class Branch
        {
            ...
            public void ApplyToCache(ILocator path, IOperation operation)
            {
                lock (this)
                    Cache.Apply(path, operation);
            }
            ...
            public void MaintenanceRoot(Token token)
            {
                if (node.IsOverflow)
                {
                    ...
                    NodeHandle = Tree.Heap.ObtainHandle();
        ...
        private class Params
        {
            ...
            public Params(WalkMethod walkMethod, WalkAction walkAction, WalkParams walkParams, bool sink, ILocator path, IData fromKey, IData toKey)
            {
